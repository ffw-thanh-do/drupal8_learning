...
class PruneImportedEntitiesFromExportTest extends EntityKernelTestBase {
   ...
   * @var string
   */
  private $this_site_origin;

  /**
   * @var string
   */
  private $untracked_unowned_uuid;

  /**
   * @var string
   */
  private $untracked_owned_uuid;

  /**
   * @var string
   */
  private $tracked_unowned_uuid;

  /**
   * @var string
   */
  private $other_site_origin;
  ...
  protected function setUp() {
    ...
    $this->this_site_origin = 'this_site_uuid';
    $this->other_site_origin = 'some-other-origin';
    $this->untracked_owned_uuid = 'some-uuid-1';
    $this->tracked_unowned_uuid = 'some-uuid-2';
    $this->untracked_unowned_uuid = 'some-uuid-3';
    ...
    $this->createUser([
      'uuid' => $this->untracked_unowned_uuid,
      'name' => 'foo',
      'mail' => 'foo@foo.com',
    ]);

    $tracked_objects = [
      $this->tracked_unowned_uuid => new CDFObject($entity_type, $this->tracked_unowned_uuid, $time, $time, $this->other_site_origin, []),
    ];

    $untracked_objects = [
      $this->untracked_owned_uuid => new CDFObject($entity_type, $this->untracked_owned_uuid, $time, $time, $this->this_site_origin, []),
      $this->untracked_unowned_uuid => new CDFObject($entity_type, $this->untracked_unowned_uuid, $time, $time, $this->other_site_origin, []),
    ];
    $untracked_objects[$this->untracked_unowned_uuid]->addAttribute('entity_type', CDFAttribute::TYPE_ARRAY_STRING, $entity_type);
    $untracked_objects[$this->untracked_unowned_uuid]->addAttribute('hash', CDFAttribute::TYPE_ARRAY_STRING, $this->other_site_origin);
    ...
    $this->event = new PrunePublishCdfEntitiesEvent($this->client->reveal(), $this->document, $this->this_site_origin);
  }

  public function testIfSiteHasDualConfigurationEntitiesOriginatedElsewhereWontBeExported(): void {
    // Expected starting state.
    $this->assertFalse($this->tracker->isTracked($this->untracked_owned_uuid));
    $this->assertTrue($this->tracker->isTracked($this->tracked_unowned_uuid));
    $this->assertFalse($this->tracker->isTracked($this->untracked_unowned_uuid));

    $this->triggerEvent($this->event);

    $document = $this->event->getDocument();
    // Subscriber created, so eligible to publish.
    $this->assertTrue($document->hasEntity($this->untracked_owned_uuid));
    // Subscriber imported, ineligible to publish.
    $this->assertFalse($document->hasEntity($this->tracked_unowned_uuid));
    $this->assertTrue($this->tracker->isTracked($this->tracked_unowned_uuid));
    // Other origin owned, ineligible to publish and should now be in tracking.
    $this->assertFalse($document->hasEntity($this->untracked_unowned_uuid));
  }

  /**
   * Triggers the onPrunePublishCdfEntitiesIfSiteHasDualConfiguration subscriber.
   *
